...
var _readrc = require("@beisen/readrc");
...
class ConfigConsumer {
  constructor(props = {}) {
    ...
    this.packageName = props.name; // sub folder bug

    this.safePackageName = props.name.replace(/\//g, '-').replace(/@/g, '');
    this.storybookConfigPath = `${configRoot}/config-${this.safePackageName}`; // copy configs
  ...
  cloneConfigs() {
    ...
    _execa2.default.sync('mkdir', [this.storybookConfigPath]); // copy config


    (0, _copyDir2.default)(oriConfigPath, this.storybookConfigPath); // override storybook config
...
const getExamples = exports.getExamples = contextRoot => {
  ...
  return (0, _fs.readdirSync)(epath) // filter directory name
  .filter(ffname => {
    let fullFFPath = _path2.default.join(epath, ffname);

    return (0, _fs.lstatSync)(fullFFPath).isDirectory();
